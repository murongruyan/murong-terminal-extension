...
fun generateBundledToolchainAssets(
    ...
    layout.symlinks
        ...
        .forEach { link ->
            ...
            commandMappings[commandName] = if (resolvedTarget.startsWith("bin/")) {
                "native/${computeBundledCommandInstallName(File(resolvedTarget).name)}"
            } else {
                resolvedTarget
            }
        }
...
fun generateBundledToolchainJniLibs(
    ...
    val executableSources = if (binDir.exists()) {
        binDir.listFiles()?.filter { it.isFile }?.sortedBy { it.name }.orEmpty()
    } else {
        emptyList()
    }
